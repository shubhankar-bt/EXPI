...
import com.shubhankaranku.expi.UiComponents.CameraPreview
import com.shubhankaranku.expi.UiComponents.GraphicsOverlay
import com.shubhankaranku.expi.Visions.FaceDetectionProcessor
...
class MainActivity : BaseActivity(), ActivityCompat.OnRequestPermissionsResultCallback, FrameReturn,
    ...
    private var emojiRatingBar: EmojiRatingBar? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        emojiRatingBar = findViewById(R.id.smile_rating)
    ...
    override fun onFrame(image: Bitmap?, face: FirebaseVisionFace?, frameMetadata: FrameMetaData?, graphicOverlay: GraphicsOverlay?) {
        ...
        if (face.smilingProbability > .9) {
            emojiRatingBar?.setCurrentRateStatus(RateStatus.GREAT)
        } else if (face.smilingProbability <= .9 && face.smilingProbability > .7) {
            emojiRatingBar?.setCurrentRateStatus(RateStatus.GOOD)
        } else if (face.smilingProbability <= .7 && face.smilingProbability > .4) {
        ...
        }else if (face.smilingProbability <= .2 && face.smilingProbability > .0) {
